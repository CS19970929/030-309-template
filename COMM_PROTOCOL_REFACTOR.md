...
- 中断只做错误清理、读取 `RDR`、写入 `ring buffer`
- 主循环再从 `ring buffer` 取字节并推进协议解析器
- 中断不再直接参与逐字节协议解析
...
- 发送前显式关闭 RX 和 `RXNE` 中断，发送完成后恢复接收，避免 TX 期间静默吞掉接收字节
- 增加 `RS485 DE/RE` 切换保护时间，降低首字节/尾字节边界时序风险
...
- 每次中断收字节时记录 `last_rx_tick`
- 主循环中通过 `bsp_CheckRunTime()` 检查距离上一字节的间隔
- 若超过当前协议超时阈值，则丢弃半包并复位解析状态机

这样可以避免半包、噪声、断帧后长期卡在接收状态。

### 并发与残包处理

本次进一步收紧了 ISR 与主循环共享状态的管理：

- `CommPortContext` 中 ISR/主循环共享的状态字段改为 `volatile`
- 接收状态机复位时同步清空 `ring buffer`
- UART 错误、协议解析错误、接收超时都会丢弃当前残包，避免旧半包污染下一帧
