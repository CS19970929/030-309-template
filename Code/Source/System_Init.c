...
void InitIO(void)
{
...

	{
		...
		GPIO_InitStructure.GPIO_Pin = PIN_KEY1; // 选择要用的GPIO引脚,PA0也可以唤醒
		GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN;
		GPIO_Init(GPIO_KEY1, &GPIO_InitStructure);
	}

	GPIO_InitStructure.GPIO_Pin = PIN_ADC_EN | PIN_CW_EN;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_Level_1;
	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;
	...
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_Level_1;
	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;
	GPIO_Init(GPIO_MCU_DRV, &GPIO_InitStructure);
	...
	GPIO_InitStructure.GPIO_Pin = PIN_LED_KEY; // 选择要用的GPIO引脚,PA0也可以唤醒
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN;
...
void CBC_TimeCtrl_Relay(void)
{
	...
	if (CBC_Element.u8CBC_CHG_ErrFlag)
	{
		...
		if (CBC_Element.u8CBC_CHG_Cnt >= 2)
		{ // 第三次进来立刻关掉全部
		...
		{
			if (++su16_CBC_CHG_TCnt > 30)
			{ // 这个不需要在下面复位，因为这个为0才有可能跳最后一个else判断
				su16_CBC_CHG_TCnt = 0;
				++CBC_Element.u8CBC_CHG_Cnt; // 打开，计一次
				CBC_Element.u8CBC_CHG_ErrFlag = 0;
			}
		}
		if (su16_CBC_Recover_TCnt)
	...
	else
	{
		...
		if (su16_CBC_CHG_TCnt)
			su16_CBC_CHG_TCnt = 0;
		if (su16_CBC_DSG_TCnt)
			su16_CBC_DSG_TCnt = 0;
	}
}
...
void CBC_Relay(void)
{
	...
	case 1:
		...
		case CLOSE:
			...
			{
				if (su16_CBCInitDelayCnt)
					su16_CBCInitDelayCnt = 0; // 突然过流之类的把其关掉了，重新计数
			}
			break;
