...
void Sci_ReadRegs_ACK(struct RS485MSG *s)
{
	...
	if (s->AckType == RS485_ACK_POS)
	{
		...
		{
			// to adapt lcd fix lcd bug
			s->u16Buffer[3] = 0;
			s->u16Buffer[4] = sleep_flag_to_lcd;
			// s->u16Buffer[13] = s->u16RdRegByteNum;
		}
		/*
...
void Sci2_CommonUpper_Tx_Deal(struct RS485MSG *s)
{
	...
	if (g_u16SCI2CommuErrCnt)
	{ // 出现奇偶校验错误等错误也得把数据全部接收完，然后不回复
		s->ptr_no = 0;
		s->csr = RS485_STA_TX_COMPLETE;
