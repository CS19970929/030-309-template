...
// 电芯与模拟量数据
typedef struct {
    uint8_t  cell_num;                   // 电芯节数M
    uint16_t cell_voltage[CELL_MAX_NUM]; // 电芯电压，单位mV
    uint8_t  temp_num;                   // 温度点数量N
    uint16_t temp_value[TEMP_MAX_NUM];   // 温度值，单位0.1K（开尔文）
    int16_t  current;                    // 电流，单位0.01A，正=充电，负=放电
    uint16_t module_voltage;             // 模块总电压，单位mV
    uint16_t remain_capacity;            // 剩余容量，单位mAh
    uint8_t  user_define_num;            // 用户自定义个数
    uint16_t total_capacity;             // 模块总容量，单位mAh
    uint16_t cycle_num;                  // 循环次数
    uint16_t full_charge_capacity;       // 满充容量
} Battery_Analog2_T;
...
typedef struct {
    ...
    Battery_Analog1_T       analog_data1;         // 电芯与模拟量数据
    Battery_Analog2_T       analog_data2;         // 电芯与模拟量数据
...
extern uint8_t tx_buf[MAX_FRAME_LEN];
