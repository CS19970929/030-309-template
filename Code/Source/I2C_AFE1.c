...
UINT8 TwiWrite_old(UINT8 SlaveID, UINT16 WrAddr, UINT8 Length, UINT8 *WrBuf)
{
	...
	if (Length > 0)
	{
		...
		{
			if (!TwiSendData(WrAddr >> 8, 0))
			{ // Send Write Address(High 8bit) For EEPROM
				goto WrErr;
			}
		}
		if (TwiSendData(WrAddr, 0))
...
UINT8 TwiRead_old(UINT8 SlaveID, UINT16 RdAddr, UINT8 Length, UINT8 *RdBuf)
{
	...
	if (Length > 0)
	{
		...
		if (SlaveID == E2PROM_ID)
		{
			if (!TwiSendData(RdAddr >> 8, 0))
			{ // Send Read Address(High 8bit) For EEPROM
				goto RdErr;
			}
		}
		if (!TwiSendData(RdAddr, 0))
...
UINT8 UpdateVoltageFromBqMaximo(void)
{
	...
	if (MTPRead(MTP_TEMP1, 42, (UINT8 *)&Registers_AFE1))
	{ // demo代码返回1为OK，
...
UINT8 UpdateVoltageFromBqMaximo_Partition(UINT8 Part)
{
	...
	{
	case 0:
		if (MTPRead(MTP_TEMP1, 10, (UINT8 *)&Registers_AFE1.Temp1))
		{ // demo代码返回1为OK，
			u32temp = ((UINT32)SH367309_Reg_Store.TR_ResRef * U16_SwapEndian(Registers_AFE1.Temp1)) / (32769 - U16_SwapEndian(Registers_AFE1.Temp1));
			SH367309_Read_AFE1.u16TempBat[0] = GetEndValue(iSheldTemp_10K_AFE, (UINT16)LENGTH_TBLTEMP_AFE_10K, u32temp);
			u32temp = ((UINT32)SH367309_Reg_Store.TR_ResRef * U16_SwapEndian(Registers_AFE1.Temp2)) / (32769 - U16_SwapEndian(Registers_AFE1.Temp2));
			...
			// 不要这个
			// SH367309_Read_AFE1.u16Current = U16_SwapEndian(Registers_AFE1.Cur1);

			SH367309_Read_AFE1.u16VCell[0] = ((UINT32)U16_SwapEndian(Registers_AFE1.Cell[0]) * 5 >> 5); ////Vcell*5/32
		}
		else
		{
			result = 1;
	...
	case 1:
		if (MTPRead(MTP_CELL2, 10, (UINT8 *)&Registers_AFE1.Cell[1]))
		{ // demo代码返回1为OK，
			for (i = 1; i < 6; i++)
			{
				SH367309_Read_AFE1.u16VCell[i] = ((UINT32)U16_SwapEndian(Registers_AFE1.Cell[i]) * 5 >> 5); ////Vcell*5/32
	...
	case 2:
		if (MTPRead(MTP_CELL7, 10, (UINT8 *)&Registers_AFE1.Cell[6]))
		{ // demo代码返回1为OK，
			for (i = 6; i < 11; i++)
			{
				SH367309_Read_AFE1.u16VCell[i] = ((UINT32)U16_SwapEndian(Registers_AFE1.Cell[i]) * 5 >> 5); ////Vcell*5/32
	...
	case 3:
		if (MTPRead(MTP_CELL12, 12, (UINT8 *)&Registers_AFE1.Cell[11]))
		{ // demo代码返回1为OK，
			for (i = 11; i < 16; i++)
			{
				SH367309_Read_AFE1.u16VCell[i] = ((UINT32)U16_SwapEndian(Registers_AFE1.Cell[i]) * 5 >> 5); ////Vcell*5/32
